...
async function execute(logger, command) {
  ...
  const code = await (0, import_exec.exec)(
    command.command,
    command.arguments,
    command.options
  );
...
function normalise(input2) {
  return {
    version: normaliseVersion(input2.version),
    directory: normaliseDirectory(input2.directory),
    access: normalisePublishAccess(input2.access),
    tag: normaliseDistributionTag(input2.tag),
    silent: normaliseBoolean(input2.silent),
    execute: normaliseBoolean(input2.execute)
  };
}
function normaliseVersion(value) {
  const refless = value.replace(/^refs\/tags\/v/, "");
  const cleaned = (0, import_coerce.default)(refless);
  if (cleaned === null) {
...
async function main(logger) {
  try {
    ...
    const options = normalise({
      version: (0, import_core.getInput)("version", { required: true }),
      directory: (0, import_core.getInput)("directory"),
      tag: (0, import_core.getInput)("tag"),
      access: (0, import_core.getInput)("private"),
      silent: (0, import_core.getInput)("silent"),
      execute: (0, import_core.getInput)("execute")
    });
    if (options.silent === true || print === void 0) {
      print = () => {
        return;
      };
    }
    ...
    (0, import_core.setOutput)("version", options.version);
  } catch (error) {
    (0, import_core.setFailed)(error);
  }
}
main(import_core.info);
