...
var normaliseInputBooleanValue = (value) => {
...
var compose = (command) => {
  ...
  return [
    command.command,
    ((_a = command == null ? void 0 : command.arguments) == null ? void 0 : _a.join(" ")) ?? []
  ].join(" ").trim();
};
var execute = async (execute3, command) => {
  const code = await execute3(
    command.command,
    command.arguments,
    command.options
  );
...
var publish = (access, tag, dryrun, options) => {
  const args = [];
